...
plotting = {
    ...
    'AIMNET2': {'width': 1.5, 'marker': 'x', 'freq': None, 'size': 3.5, 'color': '#785EF0', 'line': ':'},
    'UMA-OMOL': {'width': 1.5, 'marker': 's', 'freq': None, 'size': 3.0, 'color': '#DC267F', 'line': ':'},
    'MACE-OMOL': {'width': 1.5, 'marker': 'o', 'freq': None, 'size': 2.0, 'color': '#FFB000', 'line': ':'},
    'ORB-OMOL': {'width': 1.5, 'marker': '<', 'freq': None, 'size': 2.0, 'color': '#7B5C73', 'line': ':'}
}
...
def main():
    ...
    fig, ax = plt.subplots(1, 3, figsize=(6.4, 3.5), sharey=True)

    ax[0].set_ylabel("Relative Energy (eV)")
    ax[0].set_ylim(-5.0, 60.0)
    ...
    fig.legend(unique_labels.values(), unique_labels.keys(),
               loc='upper center', 
               bbox_to_anchor=(0.5, 0.1), # Place above the figure
               ncol=5,
               frameon=True) 

    plt.tight_layout()
    plt.subplots_adjust(bottom=0.25)
    plt.savefig('sn1_orb_3panel.png', dpi=900)
    plt.close()
