...
def setup_fonts():
    ...
    """
    plt.rcParams.update({
        'axes.titlesize': 14,         # Title of subplots
...
def main():
    ...
    fig, axes = plt.subplots(3, 2, sharex=True, sharey='row', figsize=(6.4, 8))
    ...
    axes[2, 0].set_xlabel("Internuclear Distance (Å)")
    axes[2, 1].set_xlabel("Internuclear Distance (Å)")
    ...
    fig.legend(unique_labels.values(), unique_labels.keys(),
               loc='upper center',
               bbox_to_anchor=(0.55, 0.995), # Place just below the top edge
               ncol=3,
               frameon=True,
               markerscale=2.0)
    ...
    print("Saving combo_dication_grid.png...")
    plt.savefig("combo_dication_grid.png", dpi=900)
    print("Done.")
