...
def main():
    ...
    targets = ['LiF', 'LiCl', 'LiBr', 'LiI'] 
    all_systems_data = {}

    # Pre-fetch Hydrogen Energy (constant for all)
    li_mv = get_single_atom_energy('Li', atomic_reference_data, 'mv')
    li_d3bj = get_single_atom_energy('Li', atomic_reference_data, 'd3bj')

    for sys_name in targets:
        print(f"Processing data for {sys_name}...")
        
        # Identify Halogen part (e.g. 'F' from 'HF')
        halogen = sys_name.replace('Li', '') 
        ...
        dissoc_ref = {
            'DFT': (li_mv + x_mv) * hartree2ev,
            'OMOL': (li_mv + x_mv) * hartree2ev,
            'AIMNET2': (li_d3bj + x_d3bj) * hartree2ev
        }
    ...
    if len(all_systems_data) > 0:
        make_hx_grid_plot(all_systems_data, "LiX_Neutral_Grid.png")
    else:
