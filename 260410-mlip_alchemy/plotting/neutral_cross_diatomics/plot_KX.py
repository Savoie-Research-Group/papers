...
def main():
    ...
    targets = ['KF', 'KCl', 'KBr', 'KI'] 
    all_systems_data = {}

    # Pre-fetch Hydrogen Energy (constant for all)
    k_mv = get_single_atom_energy('K', atomic_reference_data, 'mv')
    k_d3bj = get_single_atom_energy('K', atomic_reference_data, 'd3bj')

    for sys_name in targets:
        print(f"Processing data for {sys_name}...")
        
        # Identify Halogen part (e.g. 'F' from 'HF')
        halogen = sys_name.replace('K', '') 
        ...
        dissoc_ref = {
            'DFT': (k_mv + x_mv) * hartree2ev,
            'OMOL': (k_mv + x_mv) * hartree2ev,
            'AIMNET2': (k_d3bj + x_d3bj) * hartree2ev
        }
        ...
        dft_pattern = 'K_' + halogen
        mlip_pattern = "".join(sorted(['K', halogen]))
        pol_pattern = 'K' + halogen
    ...
    if len(all_systems_data) > 0:
        make_hx_grid_plot(all_systems_data, "KX_Neutral_Grid.png")
    else:
