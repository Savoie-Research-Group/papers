...
def make_hx_grid_plot(all_systems_data, plot_name):
    ...
    for molecule, (row, col) in grid_map.items():
        if molecule in all_systems_data:
            ...
            ax[row, col].set_ylim(-7.0, 15.0) # Set reasonable energy limits
        else:
...
def main():
    ...
    targets = ['HF', 'HCl', 'HBr', 'HI'] 
    all_systems_data = {}

    # Pre-fetch Hydrogen Energy (constant for all)
    h_mv = get_single_atom_energy('H', atomic_reference_data, 'mv')
    h_d3bj = get_single_atom_energy('H', atomic_reference_data, 'd3bj')

    for sys_name in targets:
        print(f"Processing data for {sys_name}...")
        
        # Identify Halogen part (e.g. 'F' from 'HF')
        halogen = sys_name.replace('H', '') 
        ...
        dissoc_ref = {
            'DFT': (h_mv + x_mv) * hartree2ev,
            'OMOL': (h_mv + x_mv) * hartree2ev,
            'AIMNET2': (h_d3bj + x_d3bj) * hartree2ev
        }
        ...
        dft_pattern = 'H_' + halogen
        mlip_pattern = "".join(sorted(['H', halogen]))
        pol_pattern = 'H' + halogen
        ...
        neutral_data = {
            ...
            'AIMNET2-NSE': mlip_scan_data.filter(regex=f'Distance_angs|({mlip_pattern}.*_charge0_.*aim2nse)'),
            'AIMNET2': mlip_scan_data.filter(regex=f'Distance_angs|({mlip_pattern}.*_charge0_.*aimnet2)'),
    ...
    if len(all_systems_data) > 0:
        make_hx_grid_plot(all_systems_data, "HX_Neutral_Grid.png")
    else:
