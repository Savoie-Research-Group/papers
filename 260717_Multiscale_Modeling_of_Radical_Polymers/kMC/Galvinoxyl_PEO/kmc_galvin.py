...
def find_sidechain_seeds(backbone_idx, adj_list, atom_types, btypes_set, ltypes_set, stypes_set):
    """
    Starting from one backbone bead, walk only through linker beads until
    sidechain beads are found. Return sidechain seed bead indices.

    Backbone beads are NOT included.
    Linker beads are traversed but NOT included.
    Sidechain beads are returned as seeds.
    """
    seeds = set()
    visited = set([backbone_idx])
    stack = list(adj_list[backbone_idx])
    ...
    while len(stack) > 0:
        ...
        if atype in stypes_set:
            seeds.add(a)

        elif atype in ltypes_set:
            for nb in adj_list[a]:
                if nb not in visited:
                    stack.append(nb)

        else:
            # stop if this is backbone or some unrelated type
            continue
...
def main(argv):
    ...
    parser.add_argument('-ltypes', dest="ltypes", default='', type=str, help='Space delimited string of linker bead types. These are traversed to connect backbone to sidechain, but are not counted as backbone or sidechain. Default: none')
    ...
    ltypes = [int(_) for _ in args.ltypes.split()] if args.ltypes.strip() else [] #BY
    qtypes=[int(_) for _ in args.qtypes.split()]
    wtypes=[int(_) for _ in args.wtypes.split()]

    btypes_set = set(btypes)
    stypes_set = set(stypes)
    ltypes_set = set(ltypes)

    if len(btypes_set & stypes_set) > 0:
        raise ValueError("btypes and stypes overlap: {}".format(sorted(list(btypes_set & stypes_set))))

    if len(btypes_set & ltypes_set) > 0:
        raise ValueError("btypes and ltypes overlap: {}".format(sorted(list(btypes_set & ltypes_set))))

    if len(stypes_set & ltypes_set) > 0:
        raise ValueError("stypes and ltypes overlap: {}".format(sorted(list(stypes_set & ltypes_set))))
    ...
    msites = []          # list of sites, each is a list of 0-based sidechain bead indices
    site2bidx = []       # site2bidx[s] = 0-based backbone bead index anchoring this super-site
    bidx2site = {}       # map: backbone bead index -> site index
    visited_sc = set()   # sidechain beads already assigned to a site

    for b in b_idx:
        # Find sidechain beads reachable from this backbone through linker beads
        seeds = find_sidechain_seeds(
            b,
            adj_list,
            objects["Atom_types"],
            btypes_set,
    ...
    print 'btypes = {}'.format(btypes)
    print 'ltypes = {}'.format(ltypes)
    print 'stypes = {}'.format(stypes)
    ...
    for tframe in range(args.first, args.last, args.every):
        ...
        #DEBUG#
        print("Average connectivity:", np.mean(conn.sum(axis=1)))
        print("Disconnected sites:", np.sum(conn.sum(axis=1) == 0))
        print("Total nonzero rates:", np.sum(w12 > 0)) 
        ...
        for i in range(args.Nkmc):
            ...
            if n_keep >= n_total:
                ...
                active_sites = list(np.random.choice(outersites_all, size=n_keep, replace=False))
                inactive_set = set(outersites_all) - set(active_sites)

            rate_thresh_per_ns = args.network_rate_thresh * 1.0e-9 #s--> ns conversion
