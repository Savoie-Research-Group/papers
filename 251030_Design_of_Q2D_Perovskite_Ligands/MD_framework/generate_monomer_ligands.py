...
def main(*args):
    ...
    print("-" * 80 + '\n')
    sys.stdout = Tee(sys.stdout, open('generate_monomer_ligands.log', 'w'))
    ...
    output_dir = "monomer_ligands_generated"
    ...
    # Calculate total number of ligands (only anchor + functional group A)
    total_ligands = len(anchor_candidates) * len(fg_candidates)
    print(f"Total monomer ligands to generate: {total_ligands}")
    print(f"  = {len(anchor_candidates)} anchors x {len(fg_candidates)} functional groups")
    print("-" * 80 + '\n')
    ...
    for anchor_idx, (anchor_name, anchor_smiles) in enumerate(zip(anchor_common_name, anchor_candidates)):
        ...
        for fg_A_idx, fg_A_name in enumerate(fg_common_name):
            ...
            # Generate ligand name (monomer: anchor + A only)
            ligand_name = f"NH3_{anchor_name}_Angew-{fg_A_name}_minimized.xyz"
            ...
            try:
                # Combine molecules (only anchor + ligand A)
                temp_mol, temp_mol_smi = _combine_monomer(
    ...
    print("\n" + "="*80)
    print("MONOMER GENERATION COMPLETE")
    print("="*80)
...
def _combine_monomer(anchor, ligand_A):
    """
    Combine anchor and one functional group (ligand_A) to form a monomer ligand.
    Structure: anchor-A (no B, no side chains)
    
    Uses marker atoms:
    - Na (11): connection points on functional groups (will be removed)
    - K (19): side chain attachment points (will be removed)
    - Cs (55): anchor attachment points
    - * (0): wildcard attachment points on anchor
    """
    
    # Step 1: Clean ligand A - remove both K and Na markers
    # K is for side chain attachment (not needed in monomer)
    # Na will be removed after we don't need it
    clean_ligand_A = Chem.RWMol(ligand_A)
    
    # Remove K (side chain marker) if present
    atoms_to_remove = []
    for at in clean_ligand_A.GetAtoms():
        if at.GetAtomicNum() == 19:  # K
            atoms_to_remove.append(at.GetIdx())
        elif at.GetAtomicNum() == 11:  # Na
            atoms_to_remove.append(at.GetIdx())
    
    # Remove atoms in reverse order to maintain indices
    for idx in sorted(atoms_to_remove, reverse=True):
        clean_ligand_A.RemoveAtom(idx)
...
def smi2xyz(smiles, xyzname, opt_option=False):
    """
