...
#             try:
#                 t_smi = return_smi(frag_E,frag_G,frag_adj,namespace=namespace)
#                 t_smi = Chem.CanonSmiles(t_smi)
#                 substring = "obabel -:'{}' -oinchikey".format(t_smi)
#                 output = subprocess.Popen(substring,shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE).communicate()[0].decode('utf-8')
#             except:
#                 substring = "obabel -imol {}_input.mol -oinchikey".format(namespace)
#                 output = subprocess.Popen(substring,shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE).communicate()[0].decode('utf-8')
...
#         try:
#             t_smile = return_smi(E,G,adj_mat,namespace=namespace)
#             t_smi = Chem.CanonSmiles(t_smi)
#             substring = "obabel -:'{}' -oinchikey".format(t_smile)
#             output = subprocess.Popen(substring,shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE).communicate()[0].decode('utf-8')
#         except:
#             substring = "obabel -imol {}_input.mol -oinchikey".format(namespace)
#             output = subprocess.Popen(substring,shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE).communicate()[0].decode('utf-8')
    ...
    for count_i,i in enumerate(sorted(final_products)):
        ...
        with open(i,"r") as f:
            for lc,lines in enumerate(f):
                ...
                if isfloat and len(fields) == 1 and float(fields[0]) == float(N): count+=1
                xyz[count]+=lines
...
def channel_classification(output_path,initial_xyz,inp_list,low_IRC_dict,N_dict,adj_dict,F_dict,reaction_dict,TS_folder=None,append=True,model='IRC_model.json',thresh=0.55,package='Gaussian',dg_thresh=None,IRC_xyz=None):
...
def read_IRC_outputs(output_path,IRCinp_list,adj_dict,hash_dict,N_dict,reaction_dict,F_dict=None,TSEne_dict=None,append=False,select=False,dg_thresh=None,folder_name='IRC-result',package='Gaussian',criterion='S'):
    ...
    for smi in reactant_smiles:
        ...
        if inchi_s not in E_dict.keys():
            ...
            if package.lower() == 'gaussian':
                ...
                substring = "python {}/utilities/xyz_to_Gaussian.py {} -o DFT/{}.gjf -eps {} -q {} -m {} -c \"{}\" -ty \"{} Opt=(maxcycles=100) Int=UltraFine SCF=QC Freq\" -t \"{} DFT\" "
                substring = substring.format('/'.join(os.getcwd().split('/')[:-1]),geo_file,inchikey,eps,q,multiplicity,"False",level,inchikey)
